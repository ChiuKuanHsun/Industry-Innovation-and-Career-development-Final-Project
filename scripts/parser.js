...
const NthuCourseParser = {
    // parse time code like "M1M2W1W2" into structured format
    parseTimeCode(timeCode) {
        const dayMap = { 'M': 1, 'T': 2, 'W': 3, 'R': 4, 'F': 5, 'S': 6, 'U': 7 };
        const slots = [];
        // Handle formats like T1T2F1F2
    ...
    parseCourseRow(row, columnIndexes) {
        ...
        if (addButton) {
            ...
            if (matches && matches[1]) {
                // Convert the parameter string 'this.form, 'arg1', 'arg2', ...' to an array ['arg1', 'arg2', ...]
                addActionArgs = matches[1].split(',').slice(1).map(arg => arg.trim().replace(/^'|'$/g, ''));
            }
        } else if (geInput) {
             // Handle the priority input box for GE courses
     ...
     * @returns {Array<Object|null>} - Array of course objects.
     */
    parseCourseTable(table) {
        ...
        rows.forEach(row => {
            ...
            if (row.cells.length > 1 && (row.querySelector('input[type="button"]') || row.querySelector('input[type="text"]'))) {
                ...
                courses.push(null); // Even if null, push to maintain index correspondence
            }
        });
        return courses;
    },

    // parse the list of enrolled courses (mainFrame)
    parseEnrolledCourses(mainFrameDoc) {
        ...
        rows.forEach(row => {
            ...
            if (row.cells.length > 4 && row.cells[1].innerText.trim()) {
                ...
                let isXClass = false;
                // Use dynamic index to get the note column data
    ...
    /**
     * @param {string} departmentId - Department ID (e.g., 'CS', 'EE', 'GEC')
     * @returns {Promise<Map<string, {enrolled: number, waiting: number}>>} - A Map where the key is the course ID and the value is an object containing enrolled and waiting counts.
     */
